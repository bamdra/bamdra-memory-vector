...
test("prepare-plugin-dist copies skills into dist", () => {
  const fixtureRoot = mkdtempSync(join(tmpdir(), "bamdra-memory-vector-dist-"));
  ...
  assert.equal(existsSync(join(repoCopy, "dist", "skills", "bamdra-memory-vector-operator", "SKILL.md")), true);
});
